...
process(){
    ...
    if [ ! -e gfwlist.txt ]; then
        _red '\nNot find gfwlist.txt.\n'
        clean_and_exit 2
    fi

    cp -f gfwlist.txt $BASE64_FILE

    # Decode
    printf 'Decode base64 GfwList... '
    ...
    printf 'Converting GfwList to '$OUT_TYPE'... '
...
ipset=/\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
        ...
        echo '# dnsmasq rules generated by awesome-openwrt gfwlist' > $OUT_TMP_FILE
